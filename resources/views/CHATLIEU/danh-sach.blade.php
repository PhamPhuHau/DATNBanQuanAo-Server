...
<div class="container-fluid pt-4 px-4">
    <div class="bg-light text-center rounded p-4">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <h6 class="mb-0">Danh sách chất liệu</h6>
            <a href="{{ route('chat-lieu.them') }}" type="button" class="btn btn-sm btn-outline-secondary">Thêm mới</a>
        </div>
        <div class="table-responsive">
            <table class="table text-start align-middle table-bordered table-hover mb-0">
                ...
                <tbody>
                    <tr>
                        <td style="width: 25%;">{{ $ChatLieu->id }}</td>
                        <td>{{ $ChatLieu->ten }}</td>
                        <td>
                            <a class="btn btn-outline-primary" href="{{ route('chat-lieu.cap-nhat', ['id' => $ChatLieu->id]) }}">Cập nhật</a>
                            <a class="btn btn-outline-danger" href="{{ route('chat-lieu.xoa', ['id' => $ChatLieu->id]) }}">Xóa</a>
                        </td>
                    </tr>
                </tbody>
                @endforeach
